...
(function () {
    ...
    const RAW_REGEX_RULES = [
        ...
        '本项目是完全非官方的'],
        [/All characters and related intellectual property featured are the exclusive property of Nintendo, Creatures Inc., and The Pokemon Company./i,
        ...
        [/^This move is restricted and only one of them can be present in the active moves at a time/i,
        '该招式受到限制，每只宝可梦只能装备一个此类招式'],
        ...
        [/^Bold: Def ▲, S.Def ▲, HP ▼/i,
        '大胆：防御 ▲，特防 ▲，生命 ▼'],
        [/^Adamant and Modest can't be rolled if they'd buff the highest offensive stat of the Pokemon, neither a nature can exceed 6 stars or result in 0 speed stars/i,
        '任何性格都不能使种族值超过 6 星，也不能使速度种族值降为 0 星。\n如果宝可梦的种族值 攻击>特攻，则无法获得固执性格；\n如果宝可梦的种族值 特攻>攻击，则无法获得内敛性格。'],
        [/^(.+) now has a (.+) nature!/i,
        '$1 现在是 $2 性格!'],
        ...
        //神兽介绍
        [/Believed to be the result of an attempt to replicate the ancestor of all Pokemon. The experiment did not achieve its intended goal, and the creature that emerged was deemed a failure./i,
        ...
        //游戏指南
        //Mod指南
        [/^Mods are optional\. The game can run with no mods installed, with a few mods, or with many mods enabled\.$/i,
        'Mod是可选的。游戏可以在没有安装Mod的情况下运行，也可以安装少量或大量Mod。'],        
        [/^Install from Workshop$/i,
        '从创意工坊安装'],        
        [/^Open Settings, click Mods, open the Workshop tab, then click Install on the mod you want\. After it is installed, enable it from the Installed tab\.$/i,
        '打开设置，点击 “Mod”，打开 “创意工坊” 标签页，然后点击你想要的Mod上的安装按钮。安装完成后，在 “已安装” 标签页中启用它。'],        
        ...
        '使用每个Mod卡片上的启用和禁用开关。有些Mod会立即生效，而有些则在刷新页面后生效。'],        
        [/^Where mods are saved$/i,
        'Mod的保存位置'],        
        [/^Imported and Workshop mods are linked to the browser storage, so they stay available even if you switch, export, or import game saves\. If you clear browser site data, imported mods can be removed\.$/i,
        '导入的Mod和创意工坊的Mod会链接到浏览器存储，因此即使切换、导出或导入游戏存档，它们仍然可用。如果你清除浏览器站点数据，导入的Mod会被移除。'],        
        //查看详情指南
        ...
        //能力值指南
        [/^Each species of Pokemon share the same base stats that determine the actual stats of a Pokemon at a given level$/i,
    ...
    const DICT = {
        ...
        //游戏指南
        "Game Guide": "游戏指南",
        "Mods: How to Add Mods": "Mods: 如何添加 Mod",
        "Mods: How to Create Mods (Coming Soon)": "Mods: 如何制作 Mod (即将推出)",
        "Inspecting": "查看详情",
        "Battle:": "战斗:",
        "Stats": "能力值",
        "Experience": "经验",
        ...
        ".mod is a renamed zip with mod.json, mod.js and optional icon.png.": ".mod文件是一个重命名的压缩包，包含mod.json、mod.js和可选的icon.png文件",
